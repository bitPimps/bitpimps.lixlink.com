...
class News
{
  ...
  function save ($dbConn)
  {
    ...
    {
      $sql =  "INSERT INTO News (catId, title, datePosted, shortDesc, body, active) VALUES ('" . $this->catId . "', '" . addslashes($this->title) . "', '" . Date("Y-m-d 00:00:00", time()) . "', '" . addslashes($this->shortDesc) . "', '" . addslashes($this->body) . "', '" . addslashes($this->active) . "')";
      $result = mysql_query($sql);
      $this->id = mysql_insert_id($dbConn);
    }
    else
    {
      $sql =  "UPDATE News SET catId='" . $this->catId . "', title='" . addslashes($this->title) . "', datePosted='" . Date("Y-m-d 00:00:00", $this->datePosted) . "', shortDesc='" . addslashes($this->shortDesc) . "', body='" . addslashes($this->body) . "', active='" . addslashes($this->active) . "' WHERE id=" . $this->id;
      $result = mysql_query($sql);
    }
    if(!$result)
    {
