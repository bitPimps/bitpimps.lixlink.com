...
class Links
{
  ...
  function save ($dbConn)
  {
    ...
    {
      $sql =  "INSERT INTO Links (orderId, url, name, description) VALUES ('" . $this->orderId . "', '" . addslashes($this->url) . "', '" . addslashes($this->name) . "', '" . addslashes($this->desc) . "')";
      $result = mysql_query($sql);
      $this->id = mysql_insert_id($dbConn);
    }
    else
    {
      $sql =  "UPDATE Links SET orderId='" . $this->orderId . "', url='" . addslashes($this->url) . "', name='" . addslashes($this->name) . "', description='" . addslashes($this->desc) . "' WHERE id=" . $this->id;
      $result = mysql_query($sql);
    }
    if(!$result)
    {
