...
<TITLE>Discussion Forum Move Topic</TITLE>
...
function CheckForm () {
	...
	//Check for a forum to move topic to
	if (document.frmMoveTopic.forum.value==""){
...
<div align="center" class="heading"><% = strTxtMoveTopic %></div>
<div align="center"><a href="display_topic_threads.asp?ForumID=<% = CInt(Request.QueryString("ForumID")) %>&TopicID=<% = CLng(Request.QueryString("TopicID")) %>&PagePosition=<% = CInt(Request.QueryString("PagePosition")) %>&ThreadPage=<% = Request.QueryString("ThreadPage") %>" target="_self"><% = strTxtReturnToDiscussionForum %></a><br>
</div>
   <form method="post" name="frmMoveTopic" action="move_topic.asp" onSubmit="return CheckForm();">
    <br>
     ...
     <tr> 
      <td height="24" width="680"> 
       ...
       <table width="100%" border="0" align="center"  height="8" cellpadding="4" cellspacing="1">
        <tr > 
         <td align="left" width="57%" height="2"  bgcolor="<% = strTableTitleColour %>" class="tHeading" background="<% = strTableTitleBgImage %>"><% = strTxtSelectTheForumYouWouldLikeForumIn %></td>
        </tr>
...
End If
%>
          <input type="hidden" name="TopicID" value="<% = CLng(Request.QueryString("TopicID")) %>">
  ...
  <table width="<% = strTableVariableWidth %>" border="0" cellspacing="0" cellpadding="1" align="center">
    <tr>
      <td align="center"> 
        <input type="submit" name="Submit" value="<% = strTxtMoveTopic %>">
      </td>
    </tr>
  </table>
</form>
