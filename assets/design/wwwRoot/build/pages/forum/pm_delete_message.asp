...
For each laryMesageID in Request.Form("chkDelete")
	...
	strSQL = "DELETE FROM tblPMMessage WHERE (((tblPMMessage.Author_ID)="  & lngLoggedInUserID & " ) AND ((tblPMMessage.PM_ID)= " & CLng(laryMesageID) & "));"
...
If Request.QueryString("pm_id") <> "" Then
	...
	'Delete the topic from the database	
	'Initalise the strSQL variable with an SQL statement to get the topic from the database
	strSQL = "DELETE FROM tblPMMessage WHERE (((tblPMMessage.Author_ID)="  & lngLoggedInUserID & " ) AND ((tblPMMessage.PM_ID)= " & CLng(Request.QueryString("pm_id")) & "));"
...
Response.Redirect "pm_inbox.asp?PagePosition=" & Request.QueryString("Page")
%>