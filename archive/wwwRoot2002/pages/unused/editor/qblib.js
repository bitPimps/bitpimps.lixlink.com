...
function checkFiletype(oform,ename)
{
  ...
  var reg= /gif|jpg|png/
  if( !reg.test(bgx) ) alert("File type isn't a image type.") 
...
{
 if(!CURRENT){ alert("Please select a element"); return }
...
{
 if(!CURRENT){ alert("Please select a control element"); return }
...
document.onmousedown= checkElement
document.onmouseup = function(){ DELTA=0 }
